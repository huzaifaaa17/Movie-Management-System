...
const API_BASE = window.location.origin + "/api/proxy";
...
function render() {
  ...
  if(addForm && user && user.admin) {
    addForm.onsubmit = function(e){
      ...
      f.actors = (f.actors || "").split(",").map(x=>x.trim()).filter(Boolean);
      ...
      ${
        ...
        <div class="movie-card">
          <div class="poster-wrap" onclick="goto('movie',${m.index})" style="cursor:pointer;">
            ...
            <img class="poster" src="${m.poster || 'https://images.unsplash.com/photo-1504384308090-c894fdcc538d?auto=format&fit=crop&w=400&q=80'}" alt="${escapeHtml(m.title)} Poster"
  ...
  return `<section class="section">
    <div class="details-page">
      <div class="details-poster">
        <img src="${m.poster || 'https://images.unsplash.com/photo-1504384308090-c894fdcc538d?auto=format&fit=crop&w=400&q=80'}" alt="" onerror="posterFallback(this)"/>
    ...
    ${wl.map(i=>`
      <div class="movie-card" onclick="goto('movie',${i})" style="cursor:pointer;">
        <div class="poster-wrap">
          <span class="card-gradient"></span>
          <img class="poster" src="${MOVIES[i].poster || 'https://images.unsplash.com/photo-1504384308090-c894fdcc538d?auto=format&fit=crop&w=400&q=80'}"
...
window.attachAuthHandlers = function() {
  // Admin login handler — verified server-side, no credentials stored client-side
    ...
    adminForm.onsubmit = async function(e) {
      ...
      try {
        const resp = await fetch(API_BASE + '/admin-login', {
          ...
          body: JSON.stringify({ adminname: f.adminname, password: f.password })
        });
        const data = await resp.json();
        if (resp.ok && data.success) {
          localStorage.setItem('nf_email', data.user.email);
          user = { email: data.user.email, admin: true };
          loginRole = null;
          if (el) el.textContent = '';
          goto('home');
          render();
        } else {
          if (el) el.textContent = data.error || 'Invalid admin credentials';
        }
      } catch (err) {
        if (el) el.textContent = 'Could not reach server. Is the backend running?';
      }
    };
  }

  // Regular user login handler — verified server-side
  ...
  if (loginForm) {
    loginForm.onsubmit = async function(e) {
      ...
      try {
        ...
        if (resp.ok && data.success) {
          localStorage.setItem('nf_email', data.user.email);
          user = { email: data.user.email, admin: false };
          // load user's watchlist if any (already preloaded from backend into localStorage)
          const allWL = JSON.parse(localStorage.getItem("nf_watchlist_USERS") || "{}");
          watchlist = allWL[data.user.email] || [];
          loginRole = null;
          if (el) el.textContent = '';
          goto('home');
          render();
        } else {
          if (el) el.textContent = data.error || 'Invalid email or password';
        }
      } catch (err) {
        if (el) el.textContent = 'Could not reach server. Is the backend running?';
      }
  ...
  // Registration handler — creates the account server-side (password is hashed there)
  ...
  if (regForm) {
    regForm.onsubmit = async function(e) {
      ...
      try {
        ...
        // initialize user's watchlist container locally
        const allWL = JSON.parse(localStorage.getItem("nf_watchlist_USERS") || "{}");
        allWL[data.user.email] = allWL[data.user.email] || [];
        localStorage.setItem("nf_watchlist_USERS", JSON.stringify(allWL));
